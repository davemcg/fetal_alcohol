...
plot_grid(gfi1_plot + theme(legend.position="none"), map13_plot, gfi_gene, map3_gene, labels = c('A','B',NULL,NULL), ncol=2, align='v', axis = 'lr', rel_heights = c(1,0.2))
```
...
# Table 2
```{r}
DMP_e %>% dplyr::select(Probe, chr, pos, Gene=Symbol, Type, logFC,FDR = adj.P.Val) %>% filter(FDR < 0.05) %>% mutate(Type = gsub('_',' ', Type)) %>% mutate(Type=gsub('hg19 genes ','',Type)) %>% mutate(Type=gsub('hg19 cpg ','',Type))
```
